...
class phRETS
{
    ...
    public function SearchQuery($resource, $class, $query = "", $optional_params = array())
    {
        ...
        while ($continue_searching) {
            ...
            $properties = array();
            //$this->error_log(print_r($xml->{"RETS-RESPONSE"}, true));
            ...
            foreach ($xml->METADATA->{'METADATA-LOOKUP_TYPE'} as $key) {
                if (!empty($key->attributes()->Lookup)) {
                    ...
                    foreach ($lookup_xml_array as $look) {
                        ...
                        $this_lookup[] = array(
                            'MetadataEntryID' => $metadataentryid, 'Value' => $value, 'ShortValue' => $shortvalue,
                            'LongValue' => $longvalue
                        );
                    }
        ...
        if ($xml->METADATA) {
            foreach ($xml->METADATA->{'METADATA-RESOURCE'}->Resource as $key => $value) {
                $this_resource["{$value->ResourceID}"] = array(
                    'ResourceID' => "{$value->ResourceID}", 'StandardName' => "{$value->StandardName}", 'VisibleName' => "{$value->VisibleName}",
            ...
            foreach ($xml->METADATA->{'METADATA-OBJECT'} as $key => $value) {
                foreach ($value->Object as $key) {
                    if (!empty($key->ObjectType)) {
                        $return_data[] = array(
                            'MetadataEntryID' => "{$key->MetadataEntryID}", 'VisibleName' => "{$key->VisibleName}", 'ObjectTimeStamp' => "{$key->ObjectTimeStamp}",
            ...
            foreach ($xml->METADATA->{'METADATA-CLASS'} as $key => $value) {
                foreach ($value->Class as $key) {
                    if (!empty($key->ClassName)) {
                        $return_data[] = array(
                            'ClassName' => "{$key->ClassName}", 'VisibleName' => "{$key->VisibleName}", 'StandardName' => "{$key->StandardName}",
    ...
    public function Connect($login_url, $username, $password, $ua_pwd = "", $ua_name = "BarcodeRealty/1.0")
    {
        ...
        $this->error_log("Connecting to RETS as '" . $username . "'...");
        ...
        if (empty($this->cookie_file)) {
            $this->cookie_file = tempnam("tmp/cookies", "phrets");
        }
